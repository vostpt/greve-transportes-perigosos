...
var fuel_layers = ['gasoline', 'diesel', 'lpg', 'none', 'without_gasoline', 'without_diesel', 'without_lpg'];
  ...
  return new Promise(function (resolve, reject) {
    ...
    $.getJSON("/storage/data/cache.json", function (data) {
      data.forEach(function (fuelStation) {
        ...
        var with_lpg = 0;
        var with_none = 0;
        var without_gasoline = 0;
        var without_diesel = 0;
        var without_lpg = 0;
        var icon = '';
        var popup_color = '';
    ...
    repa_layers.forEach(function (repa_element) {
      ...
      fuel_layers.forEach(function (fuel_element) {
        ...
        if (fuel_element.indexOf("without") == -1) {
          map.setFilter(layerID, ["all", ["==", "with_" + fuel_element, 1], ['==', 'repa', repa_value]]);
        } else {
          map.setFilter(layerID, ["all", ["==", fuel_element, 1], ['==', 'repa', repa_value]]);
        }
...
function addLayersFunctionality(layerID) {
  map.on('click', layerID, function (e) {
    ...
    var gasolineIcon = e.features[0].properties.sell_gasoline && e.features[0].properties.has_gasoline ? '<img src="/img/map/VOSTPT_FUELCRISIS_GASOLINA_500pxX500px.png"/>' : '<img class="no-gas"src="/img/map/VOSTPT_FUELCRISIS_GASOLINA_500pxX500px.png"/>';
    var dieselIcon = e.features[0].properties.sell_diesel && e.features[0].properties.has_diesel ? '<img src="/img/map/VOSTPT_FUELCRISIS_GASOLEO_500pxX500px.png"/>' : '<img class="no-gas" src="/img/map/VOSTPT_FUELCRISIS_GASOLEO_500pxX500px.png"/>';
    var lpgIcon = e.features[0].properties.sell_lpg && e.features[0].properties.has_lpg ? '<img width="75px" src="/img/map/VOSTPT_FUELCRISIS_GPL_500pxX500px.png"/>' : '<img class="no-gas" src="/img/map/VOSTPT_FUELCRISIS_GPL_500pxX500px.png"/>';
  ...
  map.on('mouseenter', layerID, function (e) {
    ...
    var coordinates = e.features[0].geometry.coordinates.slice();
    var tooltip = e.features[0].properties.tooltip; // Ensure that if the map is zoomed out such that multiple
    ...
    tooltip_popup.setLngLat(coordinates).setHTML(tooltip).addTo(map);
  });
...
module.exports = __webpack_require__(/*! /home/cvarandas/Projects/vostpt/greve-transportes-perigosos/resources/js/map.js */"./resources/js/map.js");
