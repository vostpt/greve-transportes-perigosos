...
var points = [];
var promises = [];
var customAttributions = ['Thanks to <a href="https://www.facebook.com/WazePortugal/">Waze Portugal</a> for providing important data and permission to use their services', 'This site is protected by reCAPTCHA and the Google <a href="https://policies.google.com/privacy">Privacy Policy</a> and <a href="https://policies.google.com/terms">Terms of Service</a> apply'];
var attributionControl = {
  "obj": null
};
var nagivationControl = {
  "obj": null
};
var fuel_layers = ['gasoline', 'diesel', 'lpg', 'none'];
var repa_layers = ['normal', 'sos', 'none'];
...
function loadPoints() {
  return new Promise(function (resolve, reject) {
    ...
    $.getJSON("/storage/data/cache.json", function (data) {
      data.forEach(function (fuelStation) {
        var with_gasoline = fuelStation.sell_gasoline && fuelStation.has_gasoline;
        var with_diesel = fuelStation.sell_diesel && fuelStation.has_diesel;
        var with_lpg = fuelStation.sell_lpg && fuelStation.has_lpg;
        var with_none = !with_gasoline && !with_diesel && !with_lpg;
        var icon = '';
        var popup_color = '';
        var priority = 0;
        var brand = fuelStation.brand;
...
function getAttributions() {
  ...
  var attributions = [].concat(customAttributions);
...
function updatePoints() {
  ...
  Promise.all(promises).then(function () {
    promises = [];
    repa_layers.forEach(function (repa_element) {
      var repa_value = repa_element;

      if (repa_value == "none") {
        repa_value = '';
      }

      fuel_layers.forEach(function (fuel_element) {
        var layerID = 'poi-' + repa_element + '-' + fuel_element;
    ...
    repa_layers.forEach(function (repa_element) {
      ...
      fuel_layers.forEach(function (fuel_element) {
        ...
        map.setFilter(layerID, ["all", [">", "with_" + fuel_element, 0], ['==', 'repa', repa_value]]);
      });
    });
...
function addLayersFunctionality(layerID) {
  map.on('click', layerID, function (e) {
    ...
    var gasolineIcon = e.features[0].properties.sell_gasoline && e.features[0].properties.has_gasoline ? '<img src="img/map/VOSTPT_GASPUMP_GASOLINA_500pxX500px.png"/>' : '<img class="no-gas"src="img/map/VOSTPT_GASPUMP_GASOLINA_500pxX500px.png"/>';
    var dieselIcon = e.features[0].properties.sell_diesel && e.features[0].properties.has_diesel ? '<img src="img/map/VOSTPT_GASPUMP_GASOLEO_500pxX500px.png"/>' : '<img class="no-gas" src="img/map/VOSTPT_GASPUMP_GASOLEO_500pxX500px.png"/>';
    var lpgIcon = e.features[0].properties.sell_lpg && e.features[0].properties.has_lpg ? '<img width="75px" src="img/map/VOSTPT_GASPUMP_GPL_500pxX500px.png"/>' : '<img class="no-gas" src="img/map/VOSTPT_GASPUMP_GPL_500pxX500px.png"/>';
    var fuelStationName = e.features[0].properties.name ? e.features[0].properties.name.toUpperCase() : '';
    var description = "";
    var fuelIcons = "";
    ...
    if (isHelping()) {
      description = '<div class="v-popup-content">' + '<div class="v-popup-header" style="background-color:#6bd7fc"><h5>' + e.features[0].properties.brand.toUpperCase() + '<br><small>' + fuelStationName + '</small></h5></div>' + '<div class="v-popup-body" style="background-color:#afe2fb">' + '<div class="row">' + fuelIcons + '</div>' + '<div class="row"><div class="col-md">Por favor indica que combústiveis não estão disponiveis na ' + fuelStationName + '.</div></div>' + '<div class="row"><div class="col-md">Carrega nas imagens deixando as disponiveis mais nitidas.</div></div>' + '</div>' + '<div class="v-popup-header" style="background-color:#6bd7fc"><a href="#" onclick="submitEntry(this,' + e.features[0].properties.id + ')"><h5 class="popup_submit_text">VALIDAR</h5></a></div>' + '</div>';
    } else {
      description = '<div class="v-popup-content">' + '<div class="v-popup-header" style="background-color:' + e.features[0].properties.popup_color + '"><h5>' + e.features[0].properties.brand.toUpperCase() + '<br><small>' + fuelStationName + '</small></h5></div>' + '<div class="v-popup-body">' + '<div class="row">' + fuelIcons + '</div></div>' + '<div class="v-popup-header" style="background-color:' + e.features[0].properties.popup_color + '"><h5>OBTER DIREÇÕES</h5></div>' + '<div class="v-popup-body directions"><a href="https://www.waze.com/ul?ll=' + coordinates[1] + '%2C' + coordinates[0] + '&navigate=yes&zoom=16&download_prompt=false"  target="_blank" rel="noopener noreferrer"><img src="/img/map/map_blur.png"></a></div>' + '</div>';
    }
    ...
    popup = new mapboxgl.Popup().setLngLat(coordinates).setHTML(description).addTo(map);
  });
...
function updateLayersOptions() {
  var type = $("input.type[type=radio]:checked").val();
  var repa = [];
  var repa_objects = $('input[name="fuel_stations_repa[]"]:checked');
  Object.values(repa_objects).forEach(function (repa_object) {
    var value = repa_object.value;
  ...
  repa_layers.forEach(function (repa_element) {
    fuel_layers.forEach(function (fuel_element) {
      var layerID = 'poi-' + repa_element + '-' + fuel_element;
      var repa_condition = repa.includes(repa_element);
      var fuel_condition = fuel_element == type || type == "all";
      var condition = repa_condition && fuel_condition;
...
module.exports = __webpack_require__(/*! D:\Projectos\VOST\greve-transportes-perigosos\resources\js\map.js */"./resources/js/map.js");
